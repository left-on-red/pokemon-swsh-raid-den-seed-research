...
const denHashes:Array<Array<String>> = [
...
const denInfo:Array<Array<number>> = [
    [12, 185, 977], // 16 52  ; 0
    [12, 125, 1005], // 37 64
    [12, 114, 936], // 31 90
    [12, 311, 998], // 3 51
...
export class DenLoader {
    static _dens:Map<String, Den> = new Map<String, Den>();
    static init() {
        let json = require('./../data/nests.json');
        let tables = json.Tables;
        for (let i:number = 0; i < tables.length; i++) {
            let table = tables[i];
            let hash:String = table.TableID;

            let swordRaids:Array<Raid> = new Array<Raid>(12);
            let shieldRaids:Array<Raid> = new Array<Raid>(12);

            let swordEntries = table.SwordEntries;
            let shieldEntries = table.ShieldEntries;

            for (let j:number = 0; j < 12; j++) {
                let swordEntry = swordEntries[j];
                let shieldEntry = shieldEntries[j];

                let swordAbility = swordEntry.Ability;
                let swordAltform = swordEntry.AltForm;
                let swordIVCount = swordEntry.FlawlessIVs;
                let swordGender = swordEntry.Gender;
                let swordGmax = swordEntry.IsGigantamax;
                let swordSpecies = swordEntry.Species;

                //if (swordGmax) { console.log(`${Translator.getSpecie(swordSpecies)} : ${swordSpecies}`); }

                let swordStar:Array<Boolean> = new Array<Boolean>(5);
                for (let k:number = 0; k < 5; k++) { swordStar[k] = swordEntry.Stars[k] }

                let shieldAbility = shieldEntry.Ability;
                let shieldAltform = shieldEntry.AltForm;
                let shieldIVCount = shieldEntry.FlawlessIVs;
                let shieldGender = shieldEntry.Gender;
                let shieldGmax = shieldEntry.IsGigantamax;
                let shieldSpecies = shieldEntry.Species;
                let shieldStar:Array<Boolean> = new Array<Boolean>(5);
                for (let k:number = 0; k < 5; k++) { shieldStar[k] = shieldEntry.Stars[k] }

                swordRaids[j] = new Raid(swordAbility, swordAltform, swordIVCount, swordGender, swordGmax, swordSpecies, swordStar);
                shieldRaids[j] = new Raid(shieldAbility, shieldAltform, shieldIVCount, shieldGender, shieldGmax, shieldSpecies, shieldStar);
            }

            DenLoader._dens.set(hash, new Den(swordRaids, shieldRaids));
        }
    }

    static getDen(index:number, rarity:number):Den {
        let hash:String = denHashes[index][rarity];
        if (DenLoader._dens.has(hash)) { return DenLoader._dens.get(hash) }
        return new Den([], []);
    }

    static getLocation(index:number):number { return denInfo[index][0] }

    getCoordinates(index:number):Array<number> { return [denInfo[index][1], denInfo[index][2]] }
}